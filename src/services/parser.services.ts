export interface SpecifiedFormatCall {
    duration: number
    startDate: Date
    accessCode: number
    dialedNumber: string
...
const ParseSpecifiedFormatCall = async (value: string): Promise<SpecifiedFormatCall|{ error: string }> => {
    const call: SpecifiedFormatCall = {
        duration: 0,
        startDate: new Date(),
        accessCode: 0,
        dialedNumber: "",
